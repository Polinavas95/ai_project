...
from app.api.v1.schemas import ChatRequest, ChatResponse, QuizResponse, QuizRequest
...
async def agent_dialog(request: Request, data: ChatRequest):
    ...
    user_level = user_session.get(study_topic, {}).get("user_level", UserLevel.beginner)
    ...
    giga_chat_answer, history = await dialog_agent.ainvoke(
        history=history, study_topic=study_topic, topic_context=topic_context,
...
async def agent_dialog(request: Request, data: QuizRequest):
    ...
    giga_chat_answer, history = await quiz_agent.ainvoke(
        ...
        topic_context=topic_context, study_topic=study_topic,
    )
